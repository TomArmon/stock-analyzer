    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      <div className="flex gap-2 mb-10">
        <input
          ...
          maxLength={5}
          className="max-w-xs px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button
          onClick={add}
          disabled={!input.trim()}
      ...
      {cards.length === 0 && (
        <div className="text-center py-24 text-slate-400">
          <Star size={44} className="mx-auto mb-4 text-slate-200" />
          <p className="font-medium text-slate-500">Your watchlist is empty</p>
          <p className="text-sm mt-1">Add tickers above, or star a stock from the Search page</p>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map(({ ticker, data, loading, error }) => (
          <div
            ...
            className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 cursor-pointer hover:shadow-md transition-shadow relative group"
          >
            <button
              onClick={(e) => { e.stopPropagation(); remove(ticker); }}
              className="absolute top-3 right-3 p-1 rounded-lg text-slate-200 hover:text-red-400 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all"
            >
              <X size={14} />
            </button>
            ...
            {data && !loading && (
              <>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-0.5">{data.ticker}</p>
                <p className="text-sm text-slate-500 truncate">{data.name}</p>
